...
CREATE OR REPLACE FUNCTION Search1(IN serachString VARCHAR) returns refcursor
AS $$
DECLARE
	refc1 refcursor;
BEGIN
	OPEN  refc1 FOR
	SELECT ISSUE_ID,TITLE,DESCRIPTION,CREATOR,RESOLVER,VERIFIER
	FROM A3_ISSUE
	WHERE  TITLE LIKE '%'||serachString||'%'
	ORDER BY TITLE;
	return refc1;
